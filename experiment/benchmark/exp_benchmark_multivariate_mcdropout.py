...
train = MCDropout_trainer(num_epochs = 2000, batch_size = 500, patience = 200) #Set the trainer
...
allmeanpred_train = np.zeros((num_realizations, len(ytrain), N_trials))
allvarpred_train = np.zeros((num_realizations, len(ytrain), N_trials))
...
allmeanpred_val = np.zeros((num_realizations, len(yval), N_trials))
allvarpred_val = np.zeros((num_realizations, len(yval), N_trials))
...
train = MCDropout_trainer(num_epochs = 2000, batch_size = 500, patience = 100) #Set the trainer
torch.manual_seed(21) #Must have to initialize the network parameters
model = MCDropoutNet(input_size = X_input.shape[1], hidden_size=100, dropout_prob=0.1)
...
#     torch.manual_seed(21)
    ...
    train_loss_list, val_loss_list, model = train.training(X_train_norm, y_train_norm, X_val_norm, y_val_norm, criterion, optimizer, model)
    ...
    # Inference mode
    results_train_sorted_norm = train.predict(X_train_sorted_norm, model, delta_ = 0.1, num_realizations = num_realizations)
    outputs_train_sorted_norm = results_train_sorted_norm['pi']
    results_val_sorted_norm = train.predict(X_val_sorted_norm, model, delta_ = 0.1, num_realizations = num_realizations)
    ...
    filename = f'mcdropout_performance_dgpmultivariate.pkl'
    # # Save
...
##########################################
print('---------- Finished ----------')
filenamedone = f'mcdropout_performance_dgpmultivariate.pkl'
##########################################
# # Save
