...
class SolarkstepaheadNet_exoinput(nn.Module):
    def __init__(self, lag_input_window_size=24, exo_input_window_size=12, hidden_size_common=100, hidden_size_eachstep = 100, predicted_step=1):
        ...
        self.hidden_size = hidden_size_common
        self.hidden_size_eachstep = hidden_size_eachstep
        self.predicted_step = predicted_step
        ...
        self.exo_layers1 = nn.ModuleList([
            nn.Linear(self.hidden_size + self.num_exo_input, self.hidden_size_eachstep) for _ in range(predicted_step)
        ])
        self.bn_exo_layers1 = nn.ModuleList([
            nn.BatchNorm1d(self.hidden_size_eachstep) for _ in range(predicted_step)
        ])
        
        self.exo_layers2 = nn.ModuleList([
            nn.Linear(self.hidden_size_eachstep, self.hidden_size_eachstep) for _ in range(predicted_step)
        ])
        ...
        self.output_layers = nn.ModuleList([
            nn.Linear(self.hidden_size_eachstep, 2) for _ in range(predicted_step)
        ])
