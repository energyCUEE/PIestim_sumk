...
class pinball_objective(nn.Module): # Quantile regression releasing only one quantile
    def __init__(self, quantile_ = 0.5):
        super(pinball_objective, self).__init__()
        self.quantile_ = quantile_
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
    def forward(self, y, y_pred):
        quantile = self.quantile_
        pinball_i = quantile*torch.maximum(torch.zeros_like(y).to(self.device), y - y_pred) + (1 - quantile)*torch.maximum(torch.zeros_like(y).to(self.device), -(y - y_pred))
        pinball_eachstep = torch.mean(pinball_i, axis = 0) # Averaging across samples
        pinball_allstep = torch.sum(pinball_eachstep) # Averaging across num step aheads
        
        return pinball_allstep
